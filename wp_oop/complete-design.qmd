...
Random numbers from arbitrary distributions (in this case uniform between min and max are then sampled by first creating a **distribution object** (for example a uniform distribution between 0 and 1) 
...
```
To test that this works, modify the `main` functio to print out a random number between 10 and 20.
:::
