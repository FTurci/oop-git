...
## Task 1: Create empty `bindings.cpp` file and compile with `pybind11`
...
```bash
g++ -O3 -Wall -fopenmp -shared -std=c++11 -undefined dynamic_lookup $(python -m pybind11 --includes) bindings.cpp particle.cpp system.cpp  box.cpp -o myvicsek$(python-config --extension-suffix)
```
...
:::{.callout-note icon="false"}
## Task 2: Construct the system from python
...
:::{.callout-note icon="false"}
## Task 3: Bind the rest of `System`
...
:::{.callout-note icon="false"}
## Task 5: Play with your system in Python
